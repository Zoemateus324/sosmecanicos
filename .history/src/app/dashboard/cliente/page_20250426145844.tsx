...
export default function Dashboard() {
  ...
  const { session, user, userType, isLoading } = useAuth();
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setUserNome(userType ? userType : null); // Adjust based on profiles table
  ...
  }, [user, userType, isLoading, supabase, router]);
      ...
      const fetchMechanics = async () => {
        try {
          const { data: mechanicsData, error: mechanicsError } = await supabase
            .from("users") // Adjust to match your table name
